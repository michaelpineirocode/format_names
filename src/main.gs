...
const ROW_OFFSET = 5 // if data already exists, this will offset the current number of rows to start from new submissions.
...
function placeContent(content) {
  ...
  ss.getRange((ROW_OFFSET - last_row) + 1, COL).setValue(content) // place the content in the next row of the column
}
